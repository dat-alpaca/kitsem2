...
static void student_from_file(FILE* file, buffer* const students, u64 count)
{
    ...
    {
        student current; 
        if (fscanf(file, "%f %ld", &current.gpa, &current.credits) != 2)
            break;
        fgetc(file);
...
/* core/sort.h */
static void swap(void* lhs, void* rhs, u64 size)
{
    u8* l = (u8*)lhs;
    u8* r = (u8*)rhs;
...
static void selection_sort(void* base, u64 count, student_comparison_function comparisonFunction)
{
    student* data = (student*)base;
    u64 studentSize = sizeof(student);
    ...
    for (u64 i = count - 1; i > 0; --i)
    {
        ...
        {
            if (comparisonFunction(&data[j], &data[maxIndex]) > 0)
                maxIndex = j;
        }

        swap(&data[i], &data[maxIndex], studentSize);
    }
}
...
int main(int argc, char** argv)
{
    ...
    buffer_init(&studentBuffer, count, sizeof(student));
    student_from_file(inputFile, &studentBuffer, count);
    ...
    selection_sort(studentBuffer.data, count, comparisonFunction);
    ...
    student_array_to_file(outputFile, &studentBuffer);
    
    buffer_destroy(&studentBuffer);
    fclose(inputFile);
}
