...
/* core/bst/debug.h */
static void bst_debug_print(student_bst_node* root, int depth, student_comparison_mode mode)
{
    if (root == NULL)
        return;
    ...
    if (mode == STUDENT_COMP_MODE_GPA)
        printf("-=> [GPA: %.2f, Name: %s]\n", root->data.gpa, root->data.name);
    else if (mode == STUDENT_COMP_MODE_CREDITS)
        printf("-=> [Credits: %" PRId64 ", Name: %s]\n", root->data.credits, root->data.name);
    else if (mode == STUDENT_COMP_MODE_NAME)
        printf("-=> [Name: %s]\n", root->data.name);

    bst_debug_print(root->left, depth + 1, mode);
    bst_debug_print(root->right, depth + 1, mode);
}
...
int main(int argc, char** argv)
{
    ...
    bst_debug_print(bstRoot, 0, studentComparisonMode);
